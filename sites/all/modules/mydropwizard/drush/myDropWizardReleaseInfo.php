...
class myDropWizardReleaseInfo {
  const DEFAULT_URL = 'http://updates.mydropwizard.com/files/update/v1';
  ...
  public function get($request, $refresh = FALSE) {
    if ($refresh || !isset($this->cache[$request['name']])) {
      $project_release_info = myDropWizardProject::getInstance($request, $this->getCacheDuration());
  ...
  public function clearCached(array $request) {
    ...
    $url = myDropWizardProject::buildFetchUrl($request);
