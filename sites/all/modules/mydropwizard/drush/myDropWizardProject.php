...
class myDropWizardProject {
   ...
   */
  public static function getInstance(array $request, $cache_duration = myDropWizardReleaseInfo::CACHE_LIFETIME) {
    ...
    return new myDropWizardProject($xml);
  }
  ...
  public static function buildFetchUrl(array $request) {
    // Something is always setting 'status url' to updates.drupal.org, so we
    // need to override it!
    //$status_url = isset($request['status url']) ? $request['status url'] : myDropWizardReleaseInfo::DEFAULT_URL;
    $status_url = myDropWizardReleaseInfo::DEFAULT_URL;
    return $status_url . '/' . $request['drupal_version'] . '/' . $request['name'] . '.xml';
  }
